...
{
  if (bpf_link__destroy(link_))
    LOG(V1) << "failed to destroy link for uprobe probe: "
            << strerror(errno);
}
...
{
  if (bpf_link__destroy(link_))
    LOG(V1) << "failed to destroy link for usdt probe: "
            << strerror(errno);
}
...
private:
  ...
  AttachedIntervalProbe(const Probe &probe, int prog_fd, uint64_t period_ns);
...
AttachedIntervalProbe::AttachedIntervalProbe(const Probe &probe,
                                             int prog_fd,
                                             uint64_t period_ns)
    : AttachedProbe(probe), prog_fd_(prog_fd)
{
  timer_thread_ = std::thread([this, period_ns]() {
    while (!stop_thread_.load()) {
      std::this_thread::sleep_for(std::chrono::nanoseconds(period_ns));
      if (stop_thread_.load())
        break;
      ::bpf_prog_test_run_opts(prog_fd_, nullptr);
    }
  });
}
...
AttachedIntervalProbe::~AttachedIntervalProbe()
{
  ...
  if (link_) {
    if (bpf_link__destroy(link_))
      LOG(V1) << "failed to destroy link for interval probe: "
              << strerror(errno);
  }
}
    ...
    const BpfProgram &prog)
{
  ...
  uint64_t effective_period = (period > 0) ? period : (freq > 0 ? (uint64_t)(1e9 / freq) : 1000000000ULL);
  ...
  if (perf_event_fd < 0) {
    // Fallback to pure userspace timer thread
    return std::unique_ptr<AttachedIntervalProbe>(
        new AttachedIntervalProbe(probe, prog.fd(), effective_period));
  }
